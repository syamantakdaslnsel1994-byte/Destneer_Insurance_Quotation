...
// ── Feature Master — operator edits from the hub's Feature Master tab ───────
// Additive only, deliberately kept in its own file rather than written into
// any of the four *_features_mapped.json files above: those are regenerated
// wholesale by scripts/build_*_feature_mapping.js on a re-scrape, which
// would silently wipe a manual correction or a newly added feature row if
// they lived there. insurance_hub.html merges this on top of the mapped
// data at read time (see resolveMasterPlanRows()) — this file is never
// itself touched by any build script.
const FEATURE_MASTER_PATH = path.join(__dirname, '..', 'data', 'feature_master_overrides.json');
const FEATURE_MASTER_DEFAULT = {
  _comment: 'Operator edits from the Feature Master tab. Additive only — never regenerated by the build_*_feature_mapping.js scripts, never overwritten by a re-scrape.',
  customFeatures: [],
  customPlans: { care: [], niva: [], mc: [], star: [] },
  hiddenFeatures: [],
  featureRenames: {},
  overrides: { care: {}, niva: {}, mc: {}, star: {} },
  selectedPlan: { care: null, niva: null, mc: null, star: null },
};
app.get('/feature_master_overrides.json', (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  if (!require('fs').existsSync(FEATURE_MASTER_PATH)) return res.json(FEATURE_MASTER_DEFAULT);
  res.sendFile(FEATURE_MASTER_PATH);
});
// Whole-file replace, not a patch — the client always sends its full
// current in-memory copy, so there is no partial-update merge logic here
// that two concurrent saves could race on differently.
app.post('/feature_master_overrides.json', authRequired, async (req, res) => {
  try {
    const b = req.body || {};
    if (!b || typeof b !== 'object' || Array.isArray(b))
      return res.status(400).json({ error: 'Expected a JSON object.' });
    const out = {
      _comment: FEATURE_MASTER_DEFAULT._comment,
      customFeatures: Array.isArray(b.customFeatures) ? b.customFeatures : [],
      hiddenFeatures: Array.isArray(b.hiddenFeatures) ? b.hiddenFeatures : [],
      featureRenames: (b.featureRenames && typeof b.featureRenames === 'object' && !Array.isArray(b.featureRenames)) ? b.featureRenames : {},
      customPlans: {
        care: Array.isArray(b.customPlans && b.customPlans.care) ? b.customPlans.care : [],
        niva: Array.isArray(b.customPlans && b.customPlans.niva) ? b.customPlans.niva : [],
        mc: Array.isArray(b.customPlans && b.customPlans.mc) ? b.customPlans.mc : [],
        star: Array.isArray(b.customPlans && b.customPlans.star) ? b.customPlans.star : [],
      },
      overrides: {
        care: (b.overrides && b.overrides.care) || {},
        niva: (b.overrides && b.overrides.niva) || {},
        mc: (b.overrides && b.overrides.mc) || {},
        star: (b.overrides && b.overrides.star) || {},
      },
      selectedPlan: {
        care: (b.selectedPlan && b.selectedPlan.care) || null,
        niva: (b.selectedPlan && b.selectedPlan.niva) || null,
        mc: (b.selectedPlan && b.selectedPlan.mc) || null,
        star: (b.selectedPlan && b.selectedPlan.star) || null,
      },
    };
    await fsp.mkdir(path.dirname(FEATURE_MASTER_PATH), { recursive: true });
    await fsp.writeFile(FEATURE_MASTER_PATH, JSON.stringify(out, null, 2));
    res.json({ ok: true });
  } catch (e) {
